  ...
  formatItemData = (data) => {
    ...
    if(_.get(data,"length",0)){
      data.map((item, index)=> {
        ...
        temp.dates = _.get(item, "contactTime", "");
          ...
          <div className="prospects-list-container">
            {
              ...
              <ViewList title={this.props.intl.formatMessage({id: `prospects.undone.title`})} listArray={this.formatItemData(this.props.undoneProspectsList)}>
              </ViewList>            
              : ""
            }
            {
              this.props.overProspectsList.length ? 
              <ViewList title="已填写基本信息" listArray={this.formatItemData(this.props.overProspectsList)}>
              </ViewList>            
              : ""
            }
          </div>
